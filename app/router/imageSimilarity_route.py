...
@router.put("/update")
async def update_image(missing_id: Annotated[str, Form(...)],
        img: UploadFile = File(...),
        type: int = Form(...),
        gender_id: int = Form(...),
    service: SimilarityService = Depends(get_similarity_service)):
    
    logger.info(f"Received request to update image for missing_id: {missing_id}")
    logger.info(f"Form data: type={type}, gender_id={gender_id}")
    image_bytes = await img.read()
    logger.info(f"Image received. Filename:{img.filename}, Size:{len(image_bytes)} bytes")

    try:
        logger.info("Calling service.update_face...")
        ...
        logger.info(f"Successfully updated image for missing_id: {missing_id}")
        return {"success": True, "message": f"ID '{missing_id}'가 성공적으로 수정되었습니다."}
    except HTTPException as e:
        logger.error(f"Failed to update ID {missing_id}. An error occurred: {e}.", exc_info=True)
        raise e
    except Exception as e:
        logger.error(f"unexpected error: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="서버 내부 오류가 발생했습니다.")
...
async def delete_image(missing_id: str,
                       service: SimilarityService = Depends(get_similarity_service)):
    await service.delete_face(missingId=missing_id)
    return {"success": True, "message": f"ID '{missing_id}'가 성공적으로 삭제되었습니다."}
